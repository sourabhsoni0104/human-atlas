...
import {createXRControllers,createXRHands,rayFromController,type XRControllerInput} from './xr/input';
...
import {createXRDiagnostics} from './xr/diagnostics';
...
const AnatomyScene=forwardRef<AnatomySceneHandle,Props>(function AnatomyScene({atlas,state,onSelect,onProgress,onError,onXRError,onXRSessionStart,onXRSessionEnd,onXRAction},ref){
 ...
 useEffect(()=>{
  ...
  renderer.xr.enabled=true;renderer.setPixelRatio(Math.min(devicePixelRatio,innerWidth<768?1.5:2));renderer.setClearColor('#f2f3f3',1);renderer.outputColorSpace=T.SRGBColorSpace;renderer.toneMapping=T.ACESFilmicToneMapping;renderer.toneMappingExposure=1.12;el.appendChild(renderer.domElement);
  ...
  const scene=new T.Scene(),atlasRoot=new T.Group(),camera=new T.PerspectiveCamera(34,1,.005,100),controls=new OrbitControls(camera,renderer.domElement),diagnostics=createXRDiagnostics(el);atlasRoot.name='atlasRoot';scene.add(atlasRoot);
  ...
  const markers=new T.Points(markerGeometry,markerMaterial);markers.frustumCulled=false;markers.renderOrder=10;markers.visible=false;atlasRoot.add(markers);
  ...
  const loadChunk=async(ci:number)=>{
   ...
   groups.forEach((gs,system)=>{const geometry=mergeGeometries(gs,false);if(!geometry)throw new Error('Could not assemble anatomy geometry.');geometries.push(geometry);const mesh=new T.Mesh(geometry,mats.get(system as never));mesh.frustumCulled=false;atlasRoot.add(mesh);});
  ...
  const activateInput=(input:XRControllerInput)=>{const hit=inputHit(input);if(hit.ui){xrAction.current(hit.ui.action);return;}if(hit.anatomy){setXRHover(-1);select.current(atlas.parts[hit.anatomy.index].id);}};
  const controllers=createXRControllers(renderer,scene,{onSelect:activateInput,onGrabStart:input=>manipulator.start(input),onGrabEnd:input=>manipulator.end(input)}),hands=createXRHands(renderer,scene,controllers,activateInput);manipulator=createAtlasManipulator(atlasRoot);
  ...
  const tick=(immersive=false)=>{
   ...
   if(!immersive&&(isolateKey!==lastIsolate||(s.isolate&&moving))){
   ...
   controls.enabled=!immersive;controls.enableRotate=amount<.8;controls.mouseButtons.LEFT=amount<.8?T.MOUSE.ROTATE:T.MOUSE.PAN;controls.touches.ONE=amount<.8?T.TOUCH.ROTATE:T.TOUCH.PAN;ground.visible=xrMode!=='mr'&&amount<.5&&!s.isolate;platform.visible=ring.visible=innerRing.visible=xrMode!=='mr'&&amount<.5&&!s.isolate;markers.visible=amount>.75;controls.autoRotate=!immersive&&s.rotate&&!s.isolate&&amount<.4;controls.autoRotateSpeed=.65;controls.update();if(controls.autoRotate)dirty=true;
   if(dirty||immersive){renderer.render(scene,camera);if(immersive)diagnostics.frame({renderer,mode:xrMode??'desktop',loadedChunks:loaded,totalChunks:atlas.chunks.length,visibleSystems:s.isolate&&s.selected.length?1:s.visible.length});targets=[];if(!immersive&&amount>.45){const hasSolid=atlas.parts.some((p,i)=>p.system!=='integumentary'&&data[i*4+3]>.5);atlas.parts.forEach((p,i)=>{if(data[i*4+3]<.5||(hasSolid&&p.system==='integumentary'))return;let left=Infinity,right=-Infinity,top=Infinity,bottom=-Infinity;for(let corner=0;corner<8;corner++){projected.set(p.bounds[(corner&1)?1:0][0]+data[i*4],p.bounds[(corner&2)?1:0][1]+data[i*4+1],p.bounds[(corner&4)?1:0][2]+data[i*4+2]).project(camera);const x=(projected.x+1)*el.clientWidth/2,y=(1-projected.y)*el.clientHeight/2;left=Math.min(left,x);right=Math.max(right,x);top=Math.min(top,y);bottom=Math.max(bottom,y);}projected.copy(centers[i]).add(new T.Vector3(data[i*4],data[i*4+1],data[i*4+2])).project(camera);if(projected.z< -1||projected.z>1)return;targets.push({index:i,x:(projected.x+1)*el.clientWidth/2,y:(1-projected.y)*el.clientHeight/2,left,right,top,bottom});});}dirty=false;}
  ...
  return()=>{disposed=true;abort.abort();enterXR.current=async()=>{};exitXR.current=async()=>{};switchXR.current=async()=>{};resetXRPlacement.current=()=>{};cancelAnimationFrame(frame);renderer.setAnimationLoop(null);if(xrSession)void xrSession.end();observer.disconnect();diagnostics.dispose();spatialUI.dispose();hands.dispose();controllers.dispose();controls.dispose();geometries.forEach(g=>g.dispose());materials.forEach(m=>m.dispose());scene.traverse(o=>{if(o instanceof T.Mesh&&!geometries.includes(o.geometry)){o.geometry.dispose();const ms=Array.isArray(o.material)?o.material:[o.material];ms.forEach(m=>m.dispose());}});env.dispose();partTexture.dispose();selectionTexture.dispose();markerGeometry.dispose();markerMaterial.dispose();hover.remove();renderer.dispose();renderer.domElement.remove();};
 },[atlas]);
