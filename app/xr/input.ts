...
export function createXRControllers(renderer:T.WebGLRenderer,parent:T.Object3D,callbacks:XRControllerCallbacks):XRControllerSet{
 ...
 const inputs=[0,1].map(index=>{
  ...
  const input:XRControllerInput={index,targetRay,rayLine,connected:false,grabbing:false};
  ...
  targetRay.addEventListener('disconnected',()=>{if(input.grabbing)callbacks.onGrabEnd?.(input);input.connected=false;input.grabbing=false;rayLine.visible=false;});
  targetRay.addEventListener('select',()=>{if(input.connected&&!input.grabbing)callbacks.onSelect(input);});
 ...
 const reset=()=>inputs.forEach(input=>{input.connected=false;input.grabbing=false;input.rayLine.visible=false;input.rayLine.scale.z=1.8;});
